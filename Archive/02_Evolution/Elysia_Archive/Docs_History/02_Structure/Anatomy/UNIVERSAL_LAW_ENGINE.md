...
## 4. Geometric Algebra (Rotor) [IMPLEMENTED]

4차원 회전의 복잡성을 해결하기 위해 행렬(Matrix) 대신 **기하 대수(Geometric Algebra)**의 **로터(Rotor)**를 도입했습니다.
`Core/Physics/geometric_algebra.py`에 구현된 로터는 6개의 회전 평면(xy, xz, xw, yz, yw, zw)을 효율적으로 다루며, `GyroPhysicsEngine`을 통해 영혼의 4차원 회전 운동을 정교하게 시뮬레이션합니다.
