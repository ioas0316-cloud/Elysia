...
@dataclass
class EngramAttractor:
    """
    [Engram Attractor Topology (영구 기억 인그램 중력원)]
    상위 의도장(Intent Field)의 목적으로 작용하는 영구 기억 토폴로지입니다.
    특정 위치 및 위상 좌표상에 인과적 곡률(Potential Well)을 형성하여
    하위 국소 최적해(Local Minimum / Infinite Loop)에 빠진 에이전트를 끌어당깁니다.
    """
    id: str
    name: str
    position: np.ndarray # Target coordinate (e.g. 2D/3D space or topological phase)
    intensity: float = 10.0 # Gravitational attraction force
    sigma: float = 5.0 # Radius of potential influence
    active: bool = True # Exposure toggle
...
class CausalField:
    ...
    def register_engram(self, engram: EngramAttractor):
        ...
        if not isinstance(engram.position, np.ndarray):
            engram.position = np.array(engram.position, dtype=np.float32)
        if len(engram.position) < self.dimensions:
            engram.position = np.pad(engram.position, (0, self.dimensions - len(engram.position)))
        elif len(engram.position) > self.dimensions:
            engram.position = engram.position[:self.dimensions]
        self.engrams[engram.id] = engram

    def set_engram_active(self, engram_id: str, active: bool = True):
        """Exposes or hides a specific Engram attractor to reshape field potential."""
        if engram_id in self.engrams:
            self.engrams[engram_id].active = active

    def calculate_engram_potential(self, pos: np.ndarray) -> float:
        """
        Calculates the scalar potential V_engram(pos) generated by active Engrams.
        Engrams act as potential wells (negative potential attraction).
        """
        pos_arr = np.array(pos, dtype=np.float32)
        if len(pos_arr) < self.dimensions:
            pos_arr = np.pad(pos_arr, (0, self.dimensions - len(pos_arr)))
        elif len(pos_arr) > self.dimensions:
            pos_arr = pos_arr[:self.dimensions]

        potential = 0.0
        for engram in self.engrams.values():
            if not engram.active:
                continue
            dist_sq = np.sum((pos_arr - engram.position) ** 2)
            # Gaussian potential well
            potential -= engram.intensity * np.exp(-dist_sq / (2.0 * (engram.sigma ** 2)))
        return float(potential)

    def calculate_engram_gradient(self, pos: np.ndarray) -> np.ndarray:
        """
        Calculates the top-down force vector F_engram = -grad(V_engram) pulling toward Engrams.
        """
        pos_arr = np.array(pos, dtype=np.float32)
        if len(pos_arr) < self.dimensions:
            pos_arr = np.pad(pos_arr, (0, self.dimensions - len(pos_arr)))
        ...
        force = np.zeros(self.dimensions, dtype=np.float32)
        for engram in self.engrams.values():
            if not engram.active:
                continue
            diff = engram.position - pos_arr
            dist_sq = np.sum(diff ** 2)
            # Derivative of -intensity * exp(-dist_sq / 2sigma^2) w.r.t pos_arr
            w = (engram.intensity / (engram.sigma ** 2)) * np.exp(-dist_sq / (2.0 * (engram.sigma ** 2)))
            force += diff * w
        return force
