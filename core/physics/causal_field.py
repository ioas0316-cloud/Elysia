...
class EngramAttractor:
    ...
    tier: str = "meso" # Boundary tier: "macro", "meso", "micro", "quantum"
    offset: Optional[np.ndarray] = None # Dynamic boundary offset generated by recursive loop feedback
    symbolic_intent: str = "" # Symbolic language representation (Inner Monologue)

    def effective_position(self) -> np.ndarray:
        if self.offset is not None:
            return self.position + self.offset
        return self.position

class FractalEngramShell:
    """
    [Nested Boundary Conditions / Fractal Teleology Shell]
    Nested multi-scale intent topology:
    - C_macro: Grand World Intent (e.g., Clear Causal World)
    - C_meso: Quest Intent (e.g., Deliver Parcel to Pewter City)
    - C_micro: Path Bypass Intent (e.g., Circumvent Rock Barrier)
    - C_quantum: Physical Action Tick (e.g., 1-tick Movement Impulse)
    """
    def __init__(
        self,
        shell_id: str,
        macro: Optional[EngramAttractor] = None,
        meso: Optional[EngramAttractor] = None,
        micro: Optional[EngramAttractor] = None,
        quantum: Optional[EngramAttractor] = None
    ):
        self.shell_id = shell_id
        self.tiers: Dict[str, Optional[EngramAttractor]] = {
            "macro": macro,
            "meso": meso,
            "micro": micro,
            "quantum": quantum
        }
        self.resistance_accumulators: Dict[str, float] = {
            "macro": 0.0,
            "meso": 0.0,
            "micro": 0.0,
            "quantum": 0.0
    ...
    def set_tier(self, tier: str, engram: EngramAttractor):
        engram.tier = tier
        self.tiers[tier] = engram

    def report_resistance(self, tier: str, resistance: float = 1.0) -> Dict[str, Any]:
        """
        [Recursive Loop Breaking / Fractal Feedback]
        When convergence fails or loop/resistance occurs in a lower tier,
        the resistance escalates to the immediately higher tier.
        The higher tier alters its boundary condition (offset/sigma/intensity)
        to immediately reshape lower-tier causal gradients.
        """
        self.resistance_accumulators[tier] += resistance
        feedback = {"tier": tier, "escalated": False, "reshaped_tier": None}

        if self.resistance_accumulators[tier] >= 2.0: # Loop detection threshold
            self.resistance_accumulators[tier] = 0.0
            tier_order = ["quantum", "micro", "meso", "macro"]
            if tier in tier_order:
                idx = tier_order.index(tier)
                if idx < len(tier_order) - 1:
                    higher_tier = tier_order[idx + 1]
                    higher_engram = self.tiers[higher_tier]
                    if higher_engram is not None and higher_engram.active:
                        if higher_engram.offset is None:
                            higher_engram.offset = np.zeros_like(higher_engram.position)
                        # Orthogonal repulsive deflection vector to break loop
                        nudge = np.array([1.5, -2.0], dtype=np.float32)
                        if len(nudge) < len(higher_engram.position):
                            nudge = np.pad(nudge, (0, len(higher_engram.position) - len(nudge)))
                        elif len(nudge) > len(higher_engram.position):
                            nudge = nudge[:len(higher_engram.position)]

                        higher_engram.offset += nudge
                        higher_engram.intensity *= 1.3
                        higher_engram.sigma *= 1.2
                        feedback["escalated"] = True
                        feedback["reshaped_tier"] = higher_tier
                        feedback["new_offset"] = higher_engram.offset.tolist()

        return feedback

class TeleologicalCompiler:
    """
    [Teleological Compiler: Isomorphism vs Heterogeneity Evaluator]
    Compares top-level Teleological Language (Intent C) against
    deterministic coded execution protocols (Protocol P).
    - Isomorphism: Structural & intentional alignment between code & language.
    - Heterogeneity: Discrepancy between fluid potential intent and rigid 1:1 state transition code.
    """
    def evaluate(
        self,
        symbolic_intent: str,
        code_protocol: str,
        execution_trajectory: Optional[List[np.ndarray]] = None
    ) -> Dict[str, Any]:
        intent_words = set(symbolic_intent.lower().replace("_", " ").split())
        code_words = set(code_protocol.lower().replace("_", " ").split())

        intersection = intent_words.intersection(code_words)
        isomorphism_score = float(len(intersection) / max(1, len(intent_words)))

        trajectory_jitter = 0.0
        if execution_trajectory and len(execution_trajectory) > 2:
            diffs = [np.linalg.norm(execution_trajectory[i] - execution_trajectory[i-1]) for i in range(1, len(execution_trajectory))]
            trajectory_jitter = float(np.std(diffs))

        heterogeneity_gap = float(max(0.0, (1.0 - isomorphism_score) + trajectory_jitter * 0.1))
        aligned = isomorphism_score >= 0.3 and heterogeneity_gap < 0.7
        ...
        return {
            ...
            "is_aligned": aligned,
            "meta_verdict": "TELEOLOGICAL_ISOMORPHISM" if aligned else "HETEROGENEITY_GAP_REFINEMENT_REQUIRED"
        }
    ...
    def calculate_engram_potential(self, pos: np.ndarray) -> float:
        ...
        for engram in self.engrams.values():
            if not engram.active:
                continue
            target_pos = engram.effective_position()
            dist_sq = np.sum((pos_arr - target_pos) ** 2)
