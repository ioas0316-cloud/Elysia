...
class ResonanceField(Organ):
    ...
    def scan_field_with_rotor(self, soul_rotor: 'MultiVector', sensors: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        """
        [Soul Perception]
        Scans the field using the Soul's Rotor (Orientation) and Sensors.
        This represents the Soul 'looking' at the world through the Tesseract.

        Args:
            soul_rotor: The MultiVector representing the Soul's current gaze/rotation.
            sensors: A list of sensor definitions (e.g., {"type": "frequency", "range": (400, 500)}).

        Returns:
            A list of 'Interference Patterns' (Experiences) generated by the interaction.
        """
        if Rotor is None or soul_rotor is None:
            print(f"⚠️ Soul Perception Failed: Rotor is None")
            return []

        experiences = []

        # Optimize: Only check active nodes or high energy nodes to save 1060 3GB cycles
        active_nodes = [n for n in self.nodes.values() if n.energy > 0.5]

        for node in active_nodes:
            # 1. Transform: Apply Soul's Rotor to the Node's Position/State
            # We treat the node's position/quaternion as a vector to be rotated relative to the soul
            # Or, we rotate the soul's view to match the node.
            # Conceptually: Perception = Node * Soul_Rotor

            # Convert Node Quaternion to Tuple (x, y, z, w) for rotation
            # Using Q(w, x, y, z) -> (x, y, z, w) mapping
            node_vec = (node.quaternion.x, node.quaternion.y, node.quaternion.z, node.quaternion.w)

            # Rotate the node's vector into the Soul's frame of reference
            perceived_vec = Rotor.rotate_point(node_vec, soul_rotor)

            # 2. Check Sensors (Alignment/Resonance)
            for sensor in sensors:
                is_detected = False
                strength = 0.0

                if sensor["type"] == "frequency":
                    # Frequency resonance check
                    freq_range = sensor["range"]
                    if freq_range[0] <= node.frequency <= freq_range[1]:
                        is_detected = True
                        strength = node.energy

                elif sensor["type"] == "spatial":
                    # Spatial alignment check (using perceived vector)
                    # e.g., only see things 'in front' (positive Z in perceived frame)
                    px, py, pz, pw = perceived_vec
                    if pz > 0.5: # In front
                        is_detected = True
                        strength = node.energy * pz # Depth weighting

                # 3. Generate Interference (Interaction)
                if is_detected:
                    experience = self._generate_interference_pattern(node, strength, perceived_vec)
                    experiences.append(experience)

                    # [Feedback] The act of observation (with high intensity) leaves a mark
                    if strength > 5.0:
                        node.causal_mass += 0.01 # Mild interaction trace

        return experiences

    def _generate_interference_pattern(self, node: ResonanceNode, intensity: float, perceived_vec: Tuple) -> Dict[str, Any]:
        """
        Generates the raw 'Qualia' data from the interaction.
        """
        return {
            "source_id": node.id,
            "intensity": intensity,
            "frequency": node.frequency,
            "perceived_location": perceived_vec, # (x, y, z, w) relative to Soul
            "timestamp": time.time(),
