...
class PlanningCortex:
    ...
    def develop_plan(self, goal: str) -> List[Dict[str, Any]]:
        ...
        try:
            # Mock if gemini is not configured or fails
            # response_text = generate_text(prompt)
            # For testing without keys, we might want to just mock it here if generate_text fails.
            # But let's assume generate_text handles it or we mock it in tests.
            response_text = generate_text(prompt)
            ...
            return [{"tool_name": "thought", "parameters": {"content": f"I cannot plan right now due to: {e}"}}]
    ...
    # Add create_plan and execute_plan as expected by handlers.py
    def create_plan(self, goal: str):
        steps = self.develop_plan(goal)
        # Wrap in a simple object if needed, or just return list
        class Plan:
            def __init__(self, steps):
                self.steps = [type('Step', (), {'description': s.get('tool_name')}) for s in steps]
                self.raw_steps = steps
        return Plan(steps)

    def execute_plan(self, plan):
        results = []
        for step in plan.raw_steps:
            # This would actually call action_cortex
            results.append(f"Executed {step.get('tool_name')}")
        return results
