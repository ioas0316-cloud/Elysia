...
def hyper_node(node_id: str, tags: Optional[List[str]] = None):
    """
    Decorator to mark a class as a 'HyperNode' (ComfyUI-style).
    HyperNodes are identified by a unique ID rather than a file path.
    """
    def decorator(cls):
        _NEURAL_REGISTRY[f"Node:{node_id}"] = {
            "type": "hyper_node",
            "node_id": node_id,
            "tags": tags or [],
            "class": cls
