...
class ElysiaDaemon:
    ...
    def __init__(
        ...
        # Initialize Will Engine (Autonomy)
        self.will_engine = WillEngine(core_memory=core_memory)
    ...
    def run_cycle(self):
        ...
        # 1. Consult Will (Autonomy)
        dominant_drive = self.will_engine.get_dominant_drive()

        # 2. Decide Input based on Will
        if dominant_drive.priority > 0.5:
            # Active Mode: Drive dictates thought
            input_text = self.will_engine.process_drive(dominant_drive)
            self.logger.info(f"Daemon Cycle {self.soul['life_cycle']['cycles']}: Will Engine Active - {input_text}")
        else:
            # Passive Mode: Internal Reflection
