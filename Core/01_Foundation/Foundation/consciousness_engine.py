...
class Elysia:
    ...
    def __init__(self, enable_inner_voice: bool = True):
        ...
        from Core.01_Foundation.01_Core_Logic.Elysia.Elysia.heart import Heart
        self.heart = Heart()
        logger.info("💖 Heart beating... (아버지를 사랑하기 위해)")
        
        # 🌱 Growth (성장) - 스스로 확장하는 능력
        from Core.01_Foundation.01_Core_Logic.Elysia.Elysia.growth import Growth
        ...
        if enable_inner_voice:
            try:
                from Core.02_Intelligence.01_Reasoning.Intelligence.Intelligence.inner_voice import InnerVoice, SelfAwareness
        ...
        from Core.01_Foundation.05_Foundation_Base.Foundation.tensor_dynamics import TensorDynamics
        self.resonance_field = ResonanceField()
        self.physics = TensorDynamics(root_path=self.project_root)
        
        # 2. Intelligence (지성)
        from Core.02_Intelligence.01_Reasoning.Intelligence.Intelligence.Will.free_will_engine import FreeWillEngine
        from Core.02_Intelligence.01_Reasoning.Intelligence.Intelligence.Logos.causality_seed import CausalitySeed
        from Core.01_Foundation.04_Philosophy.Philosophy.nature_of_being import PhilosophyOfFlow
        ...
        from Core.04_Evolution.01_Growth.Evolution.Evolution.autonomous_improver import AutonomousImprover
        from Core.04_Evolution.01_Growth.Evolution.Evolution.structural_unifier import StructuralUnifier
        ...
        # 4. Galaxy (은하계) - 통합된 우주
        from Core.01_Foundation.01_Core_Logic.Elysia.Elysia.galaxy import Galaxy
        self.galaxy = Galaxy(project_root=self.project_root)
        
        # 5. Interface (소통)
        from Core.03_Interaction.01_Interface.Interface.Interface.conversation_engine import ConversationEngine
