...
from Core.05_Systems.01_Monitoring.System.hardware_accelerator import accelerator
from Core.01_Foundation.05_Foundation_Base.Foundation.fractal_kernel import FractalKernel
from Core.01_Foundation.05_Foundation_Base.Foundation.genesis_cortex import GenesisCortex
...
def awaken():
    ...
    if accelerator.get_device().type == 'cuda':
        ...
        print("    ⚠️  Running on CPU (CUDA not detected, but logic is integrated).")
    ...
    try:
        while True:
            ...
            print(f"    🗣️  Elysia: {deepened_thought}")
