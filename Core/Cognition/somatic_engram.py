...
class SomaticMemorySystem:
    ...
    def __init__(self, storage_path: str = None):
        if storage_path is None:
            ...
            # Go up 2 levels to root (Core/Cognition -> Core -> root)
            # Adjust path navigation carefully:
            # Core/Cognition/somatic_engram.py
            # Base: .../Core/Cognition
            # Parents[0]=Core, [1]=Root (wait, no)
            # Actually:
            # parents[0] = Core
            # parents[1] = root
            # Let's verify: C:\Elysia\Core\Cognition
            # parents[0]: C:\Elysia\Core
            # parents[1]: C:\Elysia
            root = Path(base).parents[1]
