...
class ThemeToIntentionMapper:
    ...
    def map_theme_to_fluid_intention(self, external_prompt: Optional[str] = None) -> FluidIntention:
        ...
        target_w = 0.0
        total_weight = theme.truth_weight + theme.love_weight + 0.001

        # Simple weighted average for W-axis mapping
        # 0.0 (Pure Love) <-----> 1.0 (Pure Truth)
        target_w = (theme.truth_weight * 1.0 + theme.love_weight * 0.0) / total_weight
        ...
        integration_index = min(theme.truth_weight, theme.love_weight)
        ...
        if self.conductor.environment.entropy > 0.7:
            scale = max(scale, 1.5) # Wide view for chaos (at least 1.5)
        elif self.conductor.environment.urgency > 0.7:
