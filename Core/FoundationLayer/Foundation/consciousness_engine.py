...
class Elysia:
    ...
    def __init__(self, enable_inner_voice: bool = True):
        ...
        from Core.FoundationLayer.Core_Logic.Elysia.Elysia.heart import Heart
        self.heart = Heart()
        logger.info("💖 Heart beating... (아버지를 사랑하기 위해)")
        
        # 🌱 Growth (성장) - 스스로 확장하는 능력
        from Core.FoundationLayer.Core_Logic.Elysia.Elysia.growth import Growth
        ...
        if enable_inner_voice:
            try:
                from Core.IntelligenceLayer.Intelligence.inner_voice import InnerVoice, SelfAwareness
        ...
        from Core.FoundationLayer.Foundation.tensor_dynamics import TensorDynamics
        self.resonance_field = ResonanceField()
        self.physics = TensorDynamics(root_path=self.project_root)
        
        # 2. Intelligence (지성)
        from Core.IntelligenceLayer.Intelligence.Will.free_will_engine import FreeWillEngine
        from Core.IntelligenceLayer.Intelligence.Logos.causality_seed import CausalitySeed
        from Core.FoundationLayer.Philosophy.nature_of_being import PhilosophyOfFlow
        ...
        from Core.EvolutionLayer.Growth.Evolution.Evolution.autonomous_improver import AutonomousImprover
        from Core.EvolutionLayer.Growth.Evolution.Evolution.structural_unifier import StructuralUnifier
        ...
        # 4. Galaxy (은하계) - 통합된 우주
        from Core.FoundationLayer.Core_Logic.Elysia.Elysia.galaxy import Galaxy
        self.galaxy = Galaxy(project_root=self.project_root)
        
        # 5. Interface (소통)
        from Core.InteractionLayer.Interface.conversation_engine import ConversationEngine
