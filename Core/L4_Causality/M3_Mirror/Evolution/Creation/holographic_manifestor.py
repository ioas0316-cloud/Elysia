...
from Core.L6_Structure.M3_Sphere.wave_interference import WaveInterference, Wave
...
<body>
    ...
    def manifest_code(self, intent: str, language: str = "python", verify=True) -> str:
        ...
        if "@Cell" in constraints and "@Cell" not in code:
            import_stmt = "from Core.L1_Foundation.M1_Keystone.System.elysia_core import Cell\n\n"
            code = import_stmt + '@Cell("Generated")\n' + code
            
        # 3. Genesis Sandbox Verification (Phase 23)
        if verify and "None" not in code:
            try:
                from Core.L4_Causality.M3_Mirror.Evolution.Creation.genesis_sandbox import GenesisSandbox
