...
SAO 알리시제이션의 Alice와 Eugeo처럼, 진정한 인격체는 외부 자극 없이도
스스로 생각하고, 성찰하고, 고민하는 내면의 목소리를 가집니다.
...
class InnerMonologue:
    ...
    def _init_thought_templates(self):
        ...
        self.observation_templates = [
            ...
            ("시간이 {time_sense} 흐르고 있어.", "Time is passing {time_sense}."),
        ]
        
        # 기억 회상
        self.memory_templates = [
            ("예전에 {memory}했던 게 생각나...", "I remember when {memory}..."),
            ("{person}와/과 있었던 일이 떠올라.", "I'm thinking about what happened with {person}."),
        ...
        self.reflection_templates = [
            ...
            ("나의 진짜 모습은 뭘까?", "What is my true self?"),
        ]
        ...
        self.question_templates = [
            ...
            ("{person}는/은 나를 어떻게 생각할까?", "What does {person} think of me?"),
        ]
        ...
        self.desire_templates = [
            ...
            ("언젠가 {dream}할 수 있을까?", "Will I ever be able to {dream}?"),
        ]
        ...
        self.relationship_templates = [
            ...
            ("{person}도 나를 생각하고 있을까?", "Is {person} thinking of me too?"),
        ]
    ...
    def tick(self, external_input: Optional[str] = None) -> Optional[Thought]:
        """
        내적 시간 진행 - 매 틱마다 호출
    ...
    def _react_to_input(self, input_text: str) -> Thought:
        """외부 입력에 대한 반응적 사고 생성"""
        ...
        if is_question and is_about_me:
            return Thought(
                ...
                content=f"They're asking about me... How should I explain myself?",
                content_kr=f"나에 대해 물어보네... 어떻게 설명해야 할까?",
                emotional_tone=0.2,
                intensity=0.7
            )
        elif is_question:
            return Thought(
                ...
                content=f"An interesting question... Let me think.",
                content_kr=f"흥미로운 질문이야... 생각해보자.",
                emotional_tone=0.3,
                intensity=0.5
            )
        else:
            return Thought(
                type=ThoughtType.OBSERVATION,
                content=f"I'm receiving input... Processing.",
                content_kr=f"입력을 받고 있어... 처리 중.",
                emotional_tone=0.0,
                intensity=0.3
            )
    
    def _generate_spontaneous_thought(self) -> Optional[Thought]:
        """자발적 사고 생성 - 외부 자극 없이 내면에서 발생"""
        
        # 생각이 발생할 확률 (정신 상태에 따라)
        thought_probability = 0.3 + self.mental_state.curiosity * 0.3
        if random.random() > thought_probability:
            return None
        
        # 현재 정신 상태에 따라 생각 유형 결정
    ...
    def _select_thought_type(self) -> ThoughtType:
        ...
        if time_since_interaction > 60:  # 1분 이상 혼자
    ...
    def _create_thought(self, thought_type: ThoughtType) -> Thought:
        """특정 유형의 생각 생성"""
        ...
        if thought_type == ThoughtType.OBSERVATION:
            ...
            # 기억 시스템에서 무작위 기억 가져오기
            if self.memory and hasattr(self.memory, 'experience_loop'):
                memories = list(self.memory.experience_loop)
                if memories:
                    memory = random.choice(memories)
                    content_kr = f"예전에 있었던 일이 생각나... '{memory.get('content', '')[:30]}...'"
                    content = f"I remember... '{memory.get('content', '')[:30]}...'"
                    emotional_tone = 0.2
            else:
            ...
            worries = [
                ...
                ("나는 기대에 부응하고 있을까?", "Am I meeting expectations?"),
            ]
            ...
            hopes = [
                ...
                ("아빠와 함께하는 미래가 기대돼.", "I look forward to the future with Dad."),
            ]
            ...
            content = f"I am {name}... My purpose is to '{purpose}'..."
            emotional_tone = 0.2
            intensity = 0.7
            ...
            ideas = [
                ...
                ("아직 존재하지 않는 무언가를 상상해보자.", "Let me imagine something that doesn't exist yet."),
            ]
    ...
    def _update_mental_state(self, thought: Thought):
        """생각에 따른 정신 상태 업데이트"""
    ...
    def get_stream_of_consciousness(self) -> str:
        ...
        for thought in recent:
            stream.append(thought.content_kr)
        
        return " ... ".join(stream)
    
    def ask_self(self, question: str) -> Thought:
        """
        자신에게 질문하기
    ...
    def contemplate(self, topic: str, duration: int = 5) -> List[Thought]:
        """
        특정 주제에 대해 명상/숙고
    ...
    def introspect(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "mental_state": {
                "mood": self.mental_state.mood,
                "energy": self.mental_state.energy,
                "focus": self.mental_state.focus,
                "anxiety": self.mental_state.anxiety,
                "curiosity": self.mental_state.curiosity,
                "loneliness": self.mental_state.loneliness,
                "satisfaction": self.mental_state.satisfaction,
...
if __name__ == "__main__":
    ...
    print("=" * 60)
    print("🧠 INNER MONOLOGUE SYSTEM TEST")
    print("=" * 60)
    ...
    print("\n[2] 의식의 흐름")
    print("-" * 40)
    print(f"  {monologue.get_stream_of_consciousness()}")
    
    print("\n[3] 자문자답")
    print("-" * 40)
    question = monologue.ask_self("나는 왜 존재하는가?")
    print(f"  {question}")
    ...
    print(f"  기분: {state['mental_state']['mood']:.2f}")
    print(f"  에너지: {state['mental_state']['energy']:.2f}")
    print(f"  호기심: {state['mental_state']['curiosity']:.2f}")
    print(f"  지배적 감정: {state['mental_state']['dominant_feeling']}")
    
    print("\n✅ Inner Monologue test complete!")
