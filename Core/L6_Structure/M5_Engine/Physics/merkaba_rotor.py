...
===========================================
Core.L6_Structure.M5_Engine.Physics.merkaba_rotor
...
class Rotor(MerkabaRotor):
    """
    [Compatibility Adapter]
    Adapts the new MerkabaRotor to the old Rotor interface used by UnifiedMonad.
    """
    def __init__(self, name: str, config: RotorConfig, dna: Any = None):
        # Map config to MerkabaRotor params
        super().__init__(layer_id=config.layer_id, rpm=config.rpm)
        self.name = name
    ...
    def update(self, dt: float):
        """Adapter for spin."""
        self.spin(dt)
        self.current_rpm = self.rpm # Sync
        
    def wake(self, intensity: float):
        """Adapter for boosting energy."""
        self.rpm += intensity * 100.0
        self.field_energy += intensity
