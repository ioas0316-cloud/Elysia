...
from Core.1_Body.L6_Structure.Wave.wave_interference import WaveInterference, Wave
...
<body>
    ...
    def manifest_code(self, intent: str, language: str = "python", verify=True) -> str:
        ...
        if "@Cell" in constraints and "@Cell" not in code:
            import_stmt = "from Core.1_Body.L1_Foundation.Foundation.System.elysia_core import Cell\n\n"
            code = import_stmt + '@Cell("Generated")\n' + code
            
        # 3. Genesis Sandbox Verification (Phase 23)
        if verify and "None" not in code:
            try:
                from Core.1_Body.L4_Causality.World.Evolution.Creation.genesis_sandbox import GenesisSandbox
