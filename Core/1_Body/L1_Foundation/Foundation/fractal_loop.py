...
# Core Dependencies
try:
    from Core.1_Body.L4_Causality.fractal_causality import FractalCausalityEngine, FractalCausalNode
...
try:
    from Core.1_Body.L5_Mental.Reasoning_Core.thought_space import ThoughtSpace
except ImportError:
    ThoughtSpace = None

# LifeCycle for complete feedback loop
try:
    from Core.1_Body.L2_Metabolism.life_cycle import LifeCycle
except ImportError:
    LifeCycle = None

# GrowthJournal - visible evidence of change (uses existing SelfGovernance)
try:
    from Core.1_Body.L2_Metabolism.growth_journal import get_growth_journal
except ImportError:
