...
Usage:
    from Core.04_Evolution.01_Growth.Autonomy.self_documenter import SelfDocumenter
...
class SelfDocumenter:
    ...
    def _init_tools(self):
        """도구 초기화"""
        try:
            from Core.02_Intelligence.01_Reasoning.Cognition.codebase_introspector import get_introspector
            self.introspector = get_introspector()
        except Exception as e:
            print(f"⚠️ Introspector not available: {e}")
        
        try:
            from Core.02_Intelligence.02_Memory_Linguistics.Memory.self_discovery import SelfDiscovery
            self.discovery = SelfDiscovery()
        except Exception as e:
            print(f"⚠️ SelfDiscovery not available: {e}")
        
        try:
            from Core.02_Intelligence.01_Reasoning.Cognition.why_how_explainer import get_explainer
