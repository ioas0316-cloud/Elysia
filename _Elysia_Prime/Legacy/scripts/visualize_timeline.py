...
def draw_legend(surface, font_small):
    panel = pygame.Surface((290, 120), pygame.SRCALPHA)
    panel.fill((0, 0, 0, 140))
    items = [
        ((255, 255, 0), 'Current Node'),
        ((0, 150, 255), 'Main Branch'),
        ((100, 200, 120), 'Alt Branch'),
        ((200, 200, 220), 'Labels/Text'),
        ((255, 255, 255), 'Minimap: agents/cells'),
    ]
    y = 8
    for color, label in items:
        pygame.draw.rect(panel, color, pygame.Rect(8, y+4, 16, 16))
        text = font_small.render(label, True, (220, 220, 230))
...
parser.add_argument('--legend', choices=['on', 'off'], default='off')
...
# World 객체에 width와 height 속성을 동적으로 추가
world.width = 100
world.height = 100
...
while running:
    ...
    for event in pygame.event.get():
        ...
        if event.type == pygame.KEYDOWN:
            ...
            elif event.key == pygame.K_l: show_legend = not show_legend
