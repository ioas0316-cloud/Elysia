...
def _wrap_text(text: str, limit: int = 14) -> Sequence[str]:
    if not text:
        return []
    lines = []
    current = ""
...
def draw_speech_bubble(
    ...
    pygame.draw.polygon(
        bubble,
        (255, 255, 255, alpha),
        [(width / 2 - 6, height), (width / 2 + 6, height), (width / 2, height + 6)],
    )
    pygame.draw.rect(
        bubble,
        (80, 80, 100, alpha),
    ...
    screen.blit(
        bubble,
        (pos[0] - width / 2, pos[1] - bubble.get_height() - 12),
    )


def draw_emotion_aura(
    screen: Surface,
    pos: tuple[float, float],
    emotion: str,
    level: float,
