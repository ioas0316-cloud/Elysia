...
├── utils/           ← 공용 유틸리티 및 데이터 주입 도구
│   ├── inject_foundations.py
│   ├── system_sensory_ingestion.py
│   └── read_log.py
└── verify/          ← 단위 검증 및 코어 동작 확인 스크립트 (verify_*.py, debug_*.py)
```
...
| `verify/` | 특정 수학적/물리적 성질을 단위 검증하는 스크립트 | `causal_substitution_verify.py`, `verify_circularity.py` |
| `genesis/` | 엘리시아 자율 창생 루프 관련 실험 | `genesis.py`, `volition_genesis_loop.py` |
| `utils/` | 시스템 유지보수 및 공용 유틸리티 스크립트 | `inject_foundations.py`, `system_sensory_ingestion.py` |
