...
Purpose: ensure every experiment follows the Codex (sections 22–24), Z-axis, fractal principle, and quaternion/time-acceleration laws.
...
- Never leave mojibake (e.g., `?�`). Treat broken encoding as a bug and fix it before filing a report.
...
- “Quaternion engine” = explore multiple axes (time scales, curriculum pacing, difficulty, and world kits) simultaneously.
...
Goal: probe and update **Elysia’s growth laws**, not to showcase pretty demos.
...
Mandatory metadata (Codex §24 alignment):
...
1. Quote the macro/quaternion rules above verbatim: no tick-by-tick loops, enforce `World.set_time_scale`, `N_macro`, 1,000-year × 20-seed bundles, and branch plan logging per world kit.
2. Require analysis of the canonical logs listed in §3, focusing on self-writing length/emotion/vocabulary and caretaker-feedback alignment instead of “accuracy”.
3. Demand Codex §24 metadata in every `trial_report`: `status_history`, `execution_evidence`, `blocking_reason`, reference list, and explicit `adult_ready` statements tied to evidence.
...
- Re-audit the existing 1,000-year × 20-seed branch plans; attach `plan_status` + `blocking_reason` + caretaker audit notes before marking them “ready”.
...
- Use `scripts/experiment_report_template.py` (see repo) to auto-fill the Codex §24 metadata so caretakers on low-spec GPUs can still produce complete reports.
