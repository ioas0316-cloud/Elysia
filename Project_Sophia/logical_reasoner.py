...
class LogicalReasoner:
    ...
    def _run_causal_simulation(self, cause_entity: str, main_world: World, simulation_steps: int = 5) -> List[Thought]:
        ...
        for i, cell_id in enumerate(self.cellular_world.cell_ids):
            ...
            final_hp = self.cellular_world.hp[i] if i < len(self.cellular_world.hp) else initial_hp

            # Consider a cell "activated" if its hp significantly increased
            # and it's not the cause_entity itself.
            # Use a lower threshold for testing since mock stimulus might be simple.
            # With our mock stimulus of 100.0, even 1.0 threshold is fine.
            if final_hp > initial_hp + 1.0 and cell_id != cause_entity:
