...
class QuantumState:
    ...
    # Thermodynamics
    temperature: float = 100.0 # "Hot" by default
...
class Nucleus(QuantumState):
    ...
    def add_particle(self, particle: QuantumState):
        ...
        # Fusion heats up the nucleus
        self.temperature += particle.temperature + 50.0
        self.is_frozen = False # Thaw if it was frozen
...
class FieldEntity(QuantumState):
    ...
    def __post_init__(self):
        ...
        self.is_frozen = True # Fields are inherently static
...
class StrongForceManager:
    ...
    def calculate_force(self, p1: QuantumState, p2: QuantumState) -> np.ndarray:
        # Frozen particles still exert force (Gravity/Binding) but don't move.
...
class EntropyManager:
    """
    Manages the Thermodynamics of the system.
    Handles Cooling (Active -> Frozen) and Heating (Interaction -> Thaw).
    """
    ...
    def apply_thermodynamics(self, state: QuantumState) -> QuantumState:
        ...
        if not state.is_frozen:
            ...
            if state.temperature < self.freeze_threshold:
                ...
                state.momentum = np.zeros_like(state.momentum) # Stop motion
                # print(f"[ENTROPY] {state.name} has FROZEN into a Crystal.")
    ...
    def inject_heat(self, state: QuantumState, amount: float):
        ...
        if state.is_frozen and state.temperature > self.freeze_threshold:
            ...
            # print(f"[ENTROPY] {state.name} has THAWED and is Active.")
...
class HamiltonianSystem:
    ...
    def evolve(self, state: QuantumState, dt: float = 0.1) -> QuantumState:
        ...
        """
        # 0. Apply Thermodynamics
        state = self.entropy.apply_thermodynamics(state)

        # If Frozen, skip kinematic update (Save CPU!)
        # But still update phase (Time still passes for the soul)
        if state.is_frozen:
            total_E = self.total_energy(state) # Potential energy still exists
        ...
        if state.mass > 0:
            ...
            if isinstance(state, Nucleus):
                # Use kwarg for fields not in QuantumState but in Nucleus
                # But Nucleus just adds fields.
                # For robustness, we explicitly pass core fields
                return Nucleus(
            ...
            # Photon dynamics (Photons don't freeze, they just travel)
            V = self.potential_function(state.position)
            T = state.kinetic_energy
