...
class CoreMemory:
    ...
    Updated with 'Mind Hyperdrive' (Wormhole) Logic:
    - Experiences with high 'Rifling' (Spin) bypass the standard queue.
    - They create direct links to EssencePrinciples (Wormholes).

    Updated with 'Holographic Recursion':
    - Serialization logic now recursively handles nested emotional states (Meta-Emotions).
    """
    ...
    def add_experience(self, experience: Union[Experience, Memory, Dict[str, Any]]):
        ...
        # Ensure we have an Experience object to check rifling
        exp_obj = experience if isinstance(experience, Experience) else self._dict_to_experience(experience)

        # --- Hyperdrive Check: High Rifling creates Wormholes ---
        rifling = exp_obj.tensor.calculate_rifling()
        if rifling > 0.8:
             log_memory_action(f"HYPERDRIVE: Experience '{exp_obj.content}' has high rifling ({rifling:.2f}). Creating immediate wormhole to Essence.")
             # Skip standard distillation and burn directly into Essence Loop
             # Create a temporary identity fragment wrapper to satisfy signature
             temp_fragment = IdentityFragment(
                 timestamp=exp_obj.timestamp,
                 content=exp_obj.content,
                 type="flash_insight",
                 linked_experiences=[exp_obj.timestamp],
                 emotional_summary=exp_obj.emotional_state,
                 tensor=exp_obj.tensor,
                 wave=exp_obj.wave
             )
             self._distill_identity_to_essence([self._identity_fragment_to_dict(temp_fragment)])
    ...
    def _distill_identity_to_essence(self, fragments_data: List[Dict[str, Any]]):
        ...
        # Increase complexity (mass offset) for distilled essence
        avg_tensor.mass_offset += 0.5
    ...
    # Helper to serialize EmotionalState with nested physics objects AND nested meta-emotions
    def _emotional_state_to_dict(self, es: EmotionalState) -> Dict[str, Any]:
        d = asdict(es)
        d['tensor'] = es.tensor.to_dict()
        d['wave'] = es.wave.to_dict()
        # Recursive serialization of meta-emotions
        if es.meta_emotions:
            d['meta_emotions'] = [self._emotional_state_to_dict(me) for me in es.meta_emotions]
        return d
    ...
    def _dict_to_emotional_state(self, data: Optional[Dict[str, Any]]) -> Optional[EmotionalState]:
        ...
        # Deserialize recursive meta-emotions
        meta_emotions_data = data.get('meta_emotions', [])
        meta_emotions = [self._dict_to_emotional_state(me_data) for me_data in meta_emotions_data if me_data]
