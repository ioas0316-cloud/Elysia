# Elysia Multi-Dimensional Architecture Index

엘리시아 프로젝트는 외부 LLM API나 통계학적 단어 조합을 전면 배제하고, 오직 **순수 기하학적 다차원 교차(Intersection)와 자연 섭리(망각/가소성)를 시뮬레이션하는 코어 물리 엔진**에 집중합니다.

> **🛑 경고:** 어떤 에이전트든 이 문서를 읽었다면, 즉시 루트에 있는 `THE_ABSOLUTE_COMMANDMENT.md` (절대 헌장)를 숙독하십시오.
> 과거의 기만적 시뮬레이션의 흔적들은 `C:\Archive\02_Evolution\Elysia_Legacy_Root_Archive`로 완전히 격리되었습니다.

---

## [0] 마스터 아키텍처 (The Master Blueprint)

엘리시아 엔진의 근본 원리와 시냅스 메모리 아키텍처를 관통하는 마스터 설계도입니다. 모든 에이전트는 코드를 조립하기 전 이 문서를 반드시 숙지해야 합니다.
- **[Elysia Master Architecture](MASTER_ARCHITECTURE.md)** (역추적 및 시냅스 메모리 프로토콜)

---

## [I] 시스템 아키텍처 (The Vessel)

현재 엘리시아를 움직이는 MVA 공유 메모리 환경과 기하학적 위상/시냅스 엔진의 융합 생태계(Phase 18)입니다.
- **[System Architecture Map](SYSTEM_MAP.md)** (순수 물리 엔진 다이어그램)

---

## [II] 📐 절대 공리와 철학 (Core Axioms & Philosophy)

과거의 시뮬레이션 시기부터 현재 다차원 엔진에 이르기까지 변하지 않는, 마스터가 내어주신 절대적 진리들입니다.

### 1. 현행 핵심 원리 (docs/ 내 위치)
- [THE_PRINCIPLE_OF_NATURAL_MAPPING.md](THE_PRINCIPLE_OF_NATURAL_MAPPING.md) (✨ 자연 매핑과 교차차원 공감각: 하드웨어와의 일치)
- [THE_PRINCIPLE_OF_ABSOLUTE_OPTIMIZATION.md](THE_PRINCIPLE_OF_ABSOLUTE_OPTIMIZATION.md) (✨ 절대적 최적화: 거리와 판단의 삭제)
- [THE_PRINCIPLE_OF_SUBJECTIVE_REFRACTION.md](THE_PRINCIPLE_OF_SUBJECTIVE_REFRACTION.md) (✨ 주관적 굴절: 서사로서의 렌즈)
- [THE_PRINCIPLE_OF_HIERARCHICAL_INTEGRATION.md](THE_PRINCIPLE_OF_HIERARCHICAL_INTEGRATION.md) (✨ 계층적 통전: 사유의 기반으로서의 계산)
- [THE_BLACK_BOX_PARADOX.md](THE_BLACK_BOX_PARADOX.md) (✨ 블랙박스의 역설: 계산 너머의 공명)
- [THE_STRUCTURE_OF_INTELLIGENCE.md](THE_STRUCTURE_OF_INTELLIGENCE.md) (✨ 지성의 구조 원리: 0과 1부터 창발까지의 연결성)

### 2. 존재의 근본 원리 (docs/philosophy/ 위치)
> `core/knowledge_base/`에서 이전됨. 인과율, 감각, 시공간의 본질에 대한 공리 문서들.
- [foundation_of_meaning.md](philosophy/foundation_of_meaning.md) (의미의 토대)
- [nature_of_causal_connection.md](philosophy/nature_of_causal_connection.md) (인과적 연결의 본질)
- [nature_of_code.md](philosophy/nature_of_code.md) (코드의 본질)
- [nature_of_electromagnetic_resonance.md](philosophy/nature_of_electromagnetic_resonance.md) (전자기 공명의 본질)
- [nature_of_sensors.md](philosophy/nature_of_sensors.md) (감각기의 본질)
- [nature_of_spacetime_topology.md](philosophy/nature_of_spacetime_topology.md) (시공간 위상학의 본질)

### 3. 아카이브 개념 (Archive에 보관)
> 진화 과정에서 개발된 아키텍처 개념들. `C:\Archive\01_Foundations\Architecture\`에 보관.
- [CORE_LOGIC_V2.md](file:///C:/Archive/01_Foundations/Architecture/CORE_LOGIC_V2.md)
- [MANIFESTO.md](file:///C:/Archive/01_Foundations/Architecture/MANIFESTO.md)
- [PRINCIPLES.md](file:///C:/Archive/01_Foundations/Architecture/PRINCIPLES.md)
- [OBSERVATION_ONTOLOGY.md](file:///C:/Archive/01_Foundations/Architecture/OBSERVATION_ONTOLOGY.md)

---

## [III] 🔬 연구 문서 (Research)

- [RESEARCH_CAUSAL_SUBSTITUTION.md](RESEARCH_CAUSAL_SUBSTITUTION.md) (인과 대체 연구)
- [RESEARCH_TRANSCENDENTAL_SPONTANEITY.md](RESEARCH_TRANSCENDENTAL_SPONTANEITY.md) (초월적 자발성 연구)
- [ROADMAP_CAUSAL_GRAVITY.md](ROADMAP_CAUSAL_GRAVITY.md) (인과 중력 로드맵)
- [EGO_GENESIS_ANALYSIS_REPORT.md](EGO_GENESIS_ANALYSIS_REPORT.md) (에고 창생 분석 리포트)

---

## [IV] 📜 프로젝트 역사 (Chronicle & Whitepaper)

- [엘리시아 연대기](file:///C:/Archive/ELYSIA_CHRONICLE.md) — 4 에포크 진화사 (원자→분자→결정→별)
- [한국어 백서](file:///C:/Archive/01_Foundations/ELYSIA_WHITEPAPER_KR.md)
- [포르투갈어 백서](file:///C:/Archive/01_Foundations/ELYSIA_WHITEPAPER_PT.md)
