...
__global__ void ComputeColorFromSHKernel(
    int P,
    int deg,
    int max_coeffs,
    ...
    float dist = sqrtf(dir_orig.x * dir_orig.x + dir_orig.y * dir_orig.y + dir_orig.z * dir_orig.z);
...
__global__ void ComputeColorFromSHBackwardKernel(
    int P,
    int deg,
    int max_coeffs,
    const Float3* __restrict__ means3D,
    ...
    const Float3* __restrict__ dL_drgb,
